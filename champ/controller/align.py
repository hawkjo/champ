...
def preprocess(clargs, metadata, cache):
    ...
    initialize.update_cache(clargs.image_directory, cache)
...
def main(clargs):
    metadata, cache = initialize.load_cache_and_metadata(clargs.image_directory)
    if 'preprocessed' not in cache or not cache['preprocessed']:
