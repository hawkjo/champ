...
def plot_2d_mismatches(sequence, sequence_labels, lower_ABA_matrix, upper_ABA_matrix=None, fontsize=18, cmap='RdYlBu', normalize=False, force_full_bounds=False, show_base_legend=True, cbar_limits=None):
    dimension = 3
...
def make_example_block(parent_matrix, row, column, left_letters, bottom_letters, other_matrix=None, rotate=False):
    ...
    vmin = min(0.0, np.nanmin(data))
    vmax = max(1.0, np.nanmax(data))
    print("regular vmin/max = ", vmin, vmax)
